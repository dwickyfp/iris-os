...
export const ToolMessagePart = memo(
  ({
    ...
    useEffect(() => {
      if (!isManualToolInvocation && !isApprovalRequested) return;
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        addToolResult?.({
          tool: toolName,
          toolCallId,
          output: ManualToolConfirmTag.create({ confirm: isApprove }),
        });
      };
    ...
    const CustomToolComponent = useMemo(() => {
      ...
      if (toolName === "skill_view") {
        return (
          <SkillView
            output={result}
            loading={state === "input-streaming" || state === "input-available"}
          />
      );
      }

      if (toolName === "python_compute" && state === "output-available") {
        const sandboxResult = result as {
          exitCode?: number;
          stdout?: string;
          stderr?: string;
          durationMs?: number;
        ...
        return (
          <div className="flex flex-col fade-in duration-300 animate-in gap-2">
            <div className="p-4 rounded-lg bg-card border text-xs">
              <div className="flex items-center justify-between">
                <span className="font-medium">Python compute</span>
                <Badge variant="outline">
                  exit {sandboxResult?.exitCode ?? "?"} ·{" "}
                  {Math.round((sandboxResult?.durationMs ?? 0) / 100) / 10}s
                </Badge>
              </div>
              {artifacts.length > 0 && (
            ...
            {(sandboxResult?.stdout || sandboxResult?.stderr) && (
              <details className="group p-4 rounded-lg bg-card border text-xs">
    ...
    const mcpApp = useMemo(() => {
      const appMetadata = (part as any).toolMetadata?.app;
      if (
        !mcpServerName ||
        ...
        <MCPAppRenderer
          ...
          loadResource={(app) =>
            fetch("/api/mcp-apps/resource", {
              ...
              if (!response.ok) throw new Error("Unable to load MCP App");
            ...
            <div className="flex flex-col fade-in duration-300 animate-in">
              ...
              <div className="flex gap-2 py-2">
                ...
                <div className="w-full flex flex-col gap-2">
                  ...
                  {(isManualToolInvocation || isApprovalRequested) && (
