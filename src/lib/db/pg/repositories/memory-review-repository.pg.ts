...
import {
  ...
  contentRank,
  lexicalTerms,
} from "lib/ai/memory/lexical-search";
import {
  MEMORY_TOPIC_LABELS,
...
async function findCandidates(input: {
  ...
  const matches = await contentMatches(terms);
  const rank = await contentRank(terms);
  ...
  return [
    ...new Map(candidates.map((row) => [row.id, toCandidate(row)])).values(),
  ].slice(0, limit);
}
