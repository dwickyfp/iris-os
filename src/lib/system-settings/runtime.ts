...
export function runtimeSystemSetting(
  key: SystemSettingKey,
): SystemSettingScalar {
  return values.has(key)
    ? values.get(key)!
    : SYSTEM_SETTING_DEFINITIONS[key].default;
}

export async function refreshRuntimeSystemSettings() {
  if (refreshPromise) return refreshPromise;
  refreshPromise = import("./server")
    .then(({ systemSettingsService }) =>
      Promise.all(
        (Object.keys(SYSTEM_SETTING_DEFINITIONS) as SystemSettingKey[])
          .filter(
            (key) => SYSTEM_SETTING_DEFINITIONS[key].sensitivity === "public",
          )
          .map(
            async (key) =>
